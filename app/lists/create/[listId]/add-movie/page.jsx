// app/lists/[listId]/add-movie/page.jsx

'use client';

import React, { useState } from 'react';
import { addMovieToList } from '@/lib/api'; // Assuming you'll add addMovieToList to api.js
import { useParams } from 'next/navigation';

export default function AddMovieToList() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await addMovieToList(listId, { movieId, movieDescription });
      setMessage('Movie added to list successfully!');
      setMovieId('');
      setMovieDescription('');
    } catch (err) {
  ...
  return (
    <div className="flex h-screen bg-gray-800 text-white">
      ...
      <main className="flex-1 p-8">
        <h1 className="text-2xl font-bold mb-6">Add Movie to List</h1>
        ...
        <form onSubmit={handleSubmit} className="max-w-md">
          ...
          <div className="mb-6">
            <label className="block text-sm font-medium text-white">Movie Description</label>
            <textarea
